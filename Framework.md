...
| Security-relevant user decision is covered by the documentation and a recommendation is given (ETSI 5.12) | text |
| Every data input validation method is effective for validating the corresponding data input. The validation does not provide an indication that any data input does not protect against the processing of unexpected data inputs. ETSI 5.13 | text |
| There is no indication that user input fields are vulnerable to injection-attacks and only users with right credentials are given proper access. (ETSI 5.5) | text |
...
| Easy to use and read support material for users with limited technical knowledge. In addition the model designation shall be easy to find by several different methods, i.e. on the product itself, on the box it came in or the app
(ETSI 5.3). | text |
...
| Passwords are stored according to the minimum required security practice. | text |
| Users must be authenticated towards the device to have access to its functionalities and interfaces. (ETSI 5.5) | text |
...
| User is notified about security-updates(ETSI 5.3) | text |
...
| All cryptographics detail are configured appropriately, is not known to be vulnerable and considered best practice(ETSI 5.1, 5.5, 5.8) | text |
| Software security is appropriate for the task in regards to cryptography(ETSI 5.3) | text |
| Update mechanism cannot be misused(ETSI 5.3) | text |
...
| data, will also erase the data.(GDPR 17) (ETSI 5.11) | text |
| A clear confirmation is provided after deletions(ETSI 5.11) | text |
| Users can make use of a simple functionality to erase their user data.(ETSI 5.11) | text |
| Nothing indicates that the user data is not erased(ETSI 5.11) | text |
...
| Users have to be informed that any already processed/used personal data is still lawful, at the time withdrawal of consent is given.(GDPR 7) | text |
| If the purpose for processing personal data is no longer valid, then the controller/organization shall stop processing/usage of the data. (GDPR 11) | text |
| The user has access to an easy to understand description about how personal data is being used, by whom, and for what purposes, as well as all processes their personal data may be used in.(ETSI 5.14) | text |
| The user is informed about how to express consent (opt-in choice) to the different processes their personal data may be a part of.(ETSI 5.14) | text |
| All measurements and data of that type is processed the way the documentation describes.(ETSI  5.14) | text |
| The user shall be provided with all the data the controller/company has of the user within one month from the receipt (GDPR 12) | text |
